                        ...
                        match query.fetch_all(&pool).await {
                            Ok(rows) => {
                                ...
                                if let Err(e) = s.send(message::Response::Tables {
                                    ...
                                    let msg = format!(
                                        "查询 tables success, send back message failed：{}",
                                        e
                                    );
                                    tracing::error!(msg);
                                }
...
mod test {
    ...
    #[tokio::test]
    async fn connect_mysql_should_work() {
        let url = "mysql://root:my-secret-pw@localhost:43528/test";
        let conn = sqlx::MySqlPool::connect(url).await;
        if let Err(e) = conn {
            println!("连接失败：{:?}", e);
            return;
        }
    ...
    #[tokio::test]
    async fn connect_sqlite_should_work() {
        let url = "sqlite://target/test.db";
        ...
        let mut res = sqlx::query(sql.as_str())
            ...
            .unwrap();
        for i in 0..res.len() {
            let row = res.get(i).unwrap();
            let name: String = row.get(0);
            let datatype: String = row.get(1);
            columns.push((name, datatype));
        }
        println!("{:#?}", columns);

        let sql = format!("SELECT * FROM {}", table_name);
        let res = sqlx::query(&sql).fetch_all(&mut conn).await.unwrap();
        // print result
        for i in 0..res.len() {
            let row = res.get(i).unwrap();
            for j in 0..columns.len() {
                let (name, datatype) = &columns[j];
                let value = match datatype.as_str() {
                    "INTEGER" => row.get::<i64, _>(j).to_string(),
                    "TEXT" => row.get::<String, _>(j).to_string(),
                    _ => {
                        println!("未知类型：{}", datatype);
                        continue;
                    }
                };
                print!("{}: {} ", name, value);
